...
def get_data(setting=None):
    ...
    # obtain data in the uniform formats, which load dataset and integrate into (session, subject, trail) format
        ...
        if setting.dataset.startswith('seed') and not setting.dataset.startswith(('seediv', 'seedv')):
            ...
            print(get_shape(eeg_data))
            print(get_shape(bio_data))
            all_eeg, eeg_feature_dim, all_bio, bio_feature_dim =\
                                        multimodal_preprocess(dataset =setting.dataset,eeg_data =eeg_data, bio_data = bio_data, 
                                eeg_baseline = eeg_baseline, bio_baseline=bio_baseline,sample_rate=sample_rate, 
                                pass_band=setting.pass_band,extract_bands=setting.extract_bands,eog_bands = setting.eog_bands,emg_bands=setting.emg_bands,
                                gsr_bands = setting.gsr_bands,bvp_bands = setting.bvp_bands,resp_bands= setting.resp_bands,temp_bands = setting.temp_bands,
...
def parallel_read_seed_raw(dir_path, file):
    ...
    return trail_datas
def read_seed_multimodal(dir_path):
    '''
    output: all_eeg: (session,subject, trial, (sample_num, channels, band_feature))
            all_bio: (session,subject, trial, (sample_num, eye_feature))
            label:(session,subject, trial)  
    '''
    eeg_4s_path = dir_path + "/eeg_used_4s"
    eye_path = dir_path + '/eye_tracking_feature'
    frequency_bands=['delta', 'theta', 'alpha', 'beta', 'gamma']
    seed_train_trial_list = [[0,57], [58,115],[116,165],[166,225],[226,271],[272,319],[320,378],[379,432],[433,498]]
    seed_test_trial_list =[[0,58],[59,116],[117,174],[175,232],[233,291],[292,342]]

    eeg_files = os.listdir(eeg_4s_path)
    eye_files = os.listdir(eye_path)
    eeg_files.sort()
    eye_files.sort()   
    trials_label =[2,1,0,0,1,2,0,1,2,2,1,0,1,2,0]
    label = np.tile(trials_label, (3, 12, 1))
    num_files = len(eeg_files)


    all_eeg =[[] for _ in range(3)]
    all_eye = [[] for _ in range(3)]

    for idx1, item in enumerate(eeg_files):
        session_id = idx1 % 3
        sub_eeg = []
        npz_data = np.load(os.path.join(eeg_4s_path, item)) #type: dict
        
        train_data = pickle.loads(npz_data['train_data']) #dict contains 5 frequency bands
        test_data = pickle.loads(npz_data['test_data'])
        train_label = npz_data['train_label'].astype(int)
        test_label = npz_data['test_label'].astype(int)
        eeg_train_data = np.stack([train_data[band] for band in frequency_bands], axis = 2)
        eeg_test_data = np.stack([test_data[band] for band in frequency_bands], axis = 2)
        for index in seed_train_trial_list:
            sub_eeg.append(eeg_train_data[index[0]:index[1]+1])
        for index in seed_test_trial_list:
            sub_eeg.append(eeg_test_data[index[0]:index[1]+1])
        all_eeg[session_id].append(sub_eeg)

    for idx1, item in enumerate(eye_files):
        session_id = idx1 % 3
        eye_file = os.path.join(eye_path, item)
        eye_data = pickle.load(open(eye_file, 'rb'))
        eye_train_data = eye_data['train_data_eye']
        eye_test_data = eye_data['test_data_eye']
        sub_eye = []
        for index in seed_train_trial_list:
    ...
    '''for idx1, item in enumerate(eeg_files):
        ...
        eeg_train_data = np.stack([train_data[band] for band in frequency_bands], axis = 2)
        eeg_test_data = np.stack([test_data[band] for band in frequency_bands], axis = 2)
        all_train_eeg.append(eeg_train_data)
        all_test_eeg.append(eeg_test_data)

        num_classes = len(np.unique(train_label))
        onehot_train_label = np.zeros((len(train_label), num_classes))
        onehot_train_label[np.arange(len(train_label)), train_label] = 1
        onehot_test_label = np.zeros((len(test_label), num_classes))
        onehot_test_label[np.arange(len(test_label)), test_label] = 1

        all_train_label.append(onehot_train_label)
        all_test_label.append(onehot_test_label)

    for idx1, item in enumerate(eye_files):
        eye_file = os.path.join(eye_path, item)
        eye_data = pickle.load(open(eye_file, 'rb'))

        all_train_eye.append(eye_data['train_data_eye'])
        all_test_eye.append(eye_data['test_data_eye'])'''

    return all_eeg, all_eye, None,None, label, 200, 62, 1
...
def read_seedv_multimodal_feature(dir_path, feature_type = 'de_lds'):
    eeg_path = dir_path + '/EEG_DE_features'
    eye_path = dir_path + '/Eye_movement_features'
    eeg_file_list = os.listdir(eeg_path)
    eeg_file_list.sort()
    eye_file_list = os.listdir(eye_path)
    eye_file_list.sort()
    
    all_eeg = [[] for i in range(3)]
    all_eye = [[] for i in range(3)]
    all_label = [[] for i in range(3)]

    for idx, (eeg_file, eye_file) in enumerate(zip(eeg_file_list, eye_file_list)):
        #print(eeg_file)
        #print(eye_file)
        eeg_npz = np.load(os.path.join(eeg_path, eeg_file))
        #print(type(eeg_npz))#'numpy.lib.npyio.NpzFile'
        #print(eeg_npz.files)
        eeg_data = pickle.loads(eeg_npz['data'])#dict文件
        #print(eeg_data.keys())
        label = pickle.loads(eeg_npz['label'])#dict
        #print(label.keys())
        eye_npz = np.load(os.path.join(eye_path, eye_file))
        #print(eye_npz.files) 
        eye_data = pickle.loads(eye_npz['data'])
        sub_eeg = {0:[],1:[],2:[]}
        sub_label = {0:[],1:[],2:[]}
        sub_eye = {0:[],1:[],2:[]}
        for i in list(eeg_data.keys()):
            ses_id = i // 15
            '''print(i)
            print(eeg_data[i].shape)#18*310
            print(label[i].shape)
            print(label[i])'''
            eeg_trial = eeg_data[i].reshape(-1,62,5)
            eye_trial = eye_data[i].reshape(-1,1,33)
            sub_eeg[ses_id].append(eeg_trial)
            sub_label[ses_id].append(int(label[i][0]))
            sub_eye[ses_id].append(eye_trial)

        for i in range(len(all_eeg)):
            all_eeg[i].append(sub_eeg[i])
            all_label[i].append(sub_label[i])
            all_eye[i].append(sub_eye[i])
...
def read_deap_preprocessed(dir_path):
    ...
    # label -> (session(1), subject, trail, label)
    return data, None, label, 128, 32

def read_deap_preprocessed_multimodal(dir_path):
    ch_names = ['Fp1', 'AF3', 'F3', 'F7', 'FC5', 'FC1', 'C3', 'T7', 'CP5', 'CP1', 'P3', 'P7', 'PO3', 'O1',
                'Oz', 'Pz', 'Fp2', 'AF4', 'Fz', 'F4', 'F8', 'FC6', 'FC2', 'Cz', 'C4', 'T8', 'CP6', 'CP2',
                'P4', 'P8', 'PO4', 'O2']
    bio_channel = ['hEOG', 'vEOG', 'zEMG', 'tEMG', 'GSR', 'rsp', 'bvp', 'temp']
    ...
    for s_i, subject_file in enumerate(eeg_files):
        sub_data = pickle.load(open("{}/".format(dir_path)+subject_file, "rb"), encoding="latin")
        baseline = np.mean([sub_data['data'][:,:,i*fs:(i+1)*fs] for i in range(3)], axis=0)
        for sec in range(pre_time, end_time):
            sub_data['data'][:,:,sec*fs: (sec+1)*fs]-= baseline
        sub_eeg_data_list= []
        sub_bio_data_list = []
        sub_label_list = []
        for t_i, (trail_data, trail_label) in enumerate(zip(sub_data['data'], sub_data['labels'])):
            # trail_data shape->(channels(32eeg, 8others), raw_data)
            # trail_label shape->(labels(valence, arousal, dominance, liking))
            sub_eeg_data_list.append(trail_data[:32,pretrail:])
            sub_bio_data_list.append(trail_data[32:,pretrail:])
            sub_label_list.append(trail_label)
        eeg_data[0].append(sub_eeg_data_list)  
        bio_data[0].append(sub_bio_data_list)
        label[0].append(sub_label_list)
    #eeg_data -> (session(1), subject, trail, eeg_channel, raw_data)
    #bio_data -> (session(1), subject, trail, bio_channel, raw_data)
    #label -> (session(1), subject, trail, label)
    #output: eeg_data, bio_data, baseline,label, sampling_rate, eeg_channel, bio_channel 
    return eeg_data, bio_data, None, None,label, 128, 32, 8
