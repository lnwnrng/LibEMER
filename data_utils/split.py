...
def index_to_data_multimodal(eeg_data, bio_data, label,train_indexes,test_indexes,val_indexes,keep_dim=False):
    train_eeg_data = []
    train_bio_data = []
    train_label = []
    val_eeg_data = []
    ...
    if keep_dim:
        for train_index in train_indexes:
            train_eeg_data.append(eeg_data[train_index])
            train_bio_data.append(bio_data[train_index])
            train_label.append(label[train_index])
        for test_index in test_indexes:
            test_eeg_data.append(eeg_data[test_index])
            test_bio_data.append(bio_data[test_index])
            test_label.append(label[test_index])
        if val_indexes[0] != -1:
            for val_index in val_indexes:
                val_eeg_data.append(eeg_data[val_index])
                val_bio_data.append(bio_data[val_index])
                val_label.append(label[val_index])
    else:
        for train_index in train_indexes:
            train_eeg_data.extend(eeg_data[train_index])
            train_bio_data.extend(bio_data[train_index])
            train_label.extend(label[train_index])
        for test_index in test_indexes:
            test_eeg_data.extend(eeg_data[test_index])
            test_bio_data.extend(bio_data[test_index])
            test_label.extend(label[test_index])
        if val_indexes[0] != -1:
            for val_index in val_indexes:
                val_eeg_data.extend(eeg_data[val_index])
                val_bio_data.extend(bio_data[val_index])
                val_label.extend(label[val_index])
        train_eeg_data = np.array(train_eeg_data)
        train_bio_data = np.array(train_bio_data)
        train_label = np.array(train_label)
        test_eeg_data = np.array(test_eeg_data)
        test_bio_data = np.array(test_bio_data)
        test_label = np.array(test_label)
        val_eeg_data = np.array(val_eeg_data)
        val_bio_data = np.array(val_bio_data)
        val_label = np.array(val_label)
    return train_eeg_data, train_bio_data,train_label, val_eeg_data,val_bio_data,val_label, test_eeg_data,test_bio_data, test_label
...
def get_split_index(data, label, setting=None):
    ...
    elif setting.split_type == "leave_one_out":
        loo = LeaveOneOut()
    ...
    elif setting.split_type == "front_back":
    ...
    elif setting.split_type == "train_val_test":
...
def merge_to_part(data, label,setting=None):
    ...
    m_data = []
    m_label = []
    if setting.experiment_mode == "sub_dependent" and setting.cross_trail == 'true':

        m_data = [[] for _ in range(len(data[0]) * len(sessions))]#有subject*session个索引
        m_label = [[] for _ in range(len(data[0]) * len(sessions))]
        for i in sessions:
            for idx1, subject in enumerate(data[i]):
                for idx2, trail in enumerate(subject):
                    m_data[i*len(data[i])+idx1].append(trail)#第i个session的第idx1个subject的第idx2个trail，每个索引包含每个受试的所有trial
        for i in sessions:
            for idx1, subject in enumerate(label[i]):
                for idx2, trail in enumerate(subject):
                    m_label[i*len(data[i])+idx1].append(trail)
    elif setting.experiment_mode == "sub_dependent" and setting.cross_trail == 'false':#在cross_trail为False时，只使用session1
    ...
    elif setting.experiment_mode == "sub_independent":
        m_data = [[[] for _ in range(len(data[0]))]]
        m_label = [[[] for _ in range(len(data[0]))]]
        for i in sessions:
    ...
    elif setting.experiment_mode == "cross_session":
...
def merge_to_part_multimodal(eeg_data, bio_data, label,setting=None):
    assert setting.sessions is None or (max(setting.sessions)<=len(label) and min(setting.sessions) >= 0), \
        "sessions set fault, session not exist in dataset"
    if setting.sessions is None:
        sessions = range(len(eeg_data))
    else:
        sessions = [i - 1 for i in setting.sessions]
    m_eeg_data = []
    m_bio_data = []
    ...
    if setting.experiment_mode == "sub_dependent" and setting.cross_trail == 'true':
        ...
        m_eeg_data = [[] for _ in range(len(eeg_data[0]) * len(sessions))]#有subject*session个索引
        m_bio_data = [[] for _ in range(len(bio_data[0]) * len(sessions))]
        m_label = [[] for _ in range(len(label[0]) * len(sessions))]
        for i in sessions:
            for idx1, (sub_eeg,sub_bio,sub_label) in enumerate(zip(eeg_data[i],bio_data[i],label[i] )):
                for idx2, (trial_eeg,trial_bio,trial_label) in enumerate(zip(sub_eeg,sub_bio,sub_label)):
                    m_eeg_data[i*len(eeg_data[i])+idx1].append(trial_eeg)#第i个session的第idx1个subject的第idx2个trail，每个索引包含每个受试的所有trial
                    m_bio_data[i*len(bio_data[i])+idx1].append(trial_bio)
                    m_label[i*len(label[i])+idx1].append(trial_label) #(session*subject)*trial*sample
    elif setting.experiment_mode == "sub_dependent" and setting.cross_trail == 'false':#在cross_trail为False时，只使用session1
        assert max(setting.sessions) <=1, 'sessions out of range' 
        m_eeg_data = [[] for _ in range(len(eeg_data[0]))]
        m_bio_data = [[] for _ in range(len(bio_data[0]))]
        m_label = [[] for _ in range(len(eeg_data[0]))]

        for i in sessions:
            for idx1, (sub_eeg, sub_bio, sub_label) in enumerate(zip(eeg_data[i],bio_data[i],label[i])):
                for idx2, (trail_eeg, trail_bio, trial_label) in enumerate(zip(sub_eeg, sub_bio, sub_label)):
                    for idx3, (sample_eeg, sample_bio, sample_label) in enumerate(zip(trail_eeg, trail_bio, trial_label)):
                        m_eeg_data[i*len(eeg_data[0])+idx1].append([sample_eeg])
                        m_bio_data[i*len(bio_data[0])+idx1].append([sample_bio])
                        m_label[i*len(label[0])+idx1].append([sample_label])
       
    elif setting.experiment_mode == "sub_independent":
        m_eeg_data = [[[] for _ in range(len(eeg_data[0]))]]
        m_bio_data = [[[] for _ in range(len(bio_data[0]))]]
        m_label = [[[] for _ in range(len(label[0]))]]
        for i in sessions:
            for idx, (sub_eeg,sub_bio,sub_label) in enumerate(zip(eeg_data[i],bio_data[i],label[i])):
                for idx2,(trail_eeg,trial_bio,trial_label) in enumerate(zip(sub_eeg,sub_bio,sub_label)):
                    m_eeg_data[0][idx].extend(trail_eeg)
                    m_bio_data[0][idx].extend(trial_bio)
                    m_label[0][idx].extend(trial_label)#1*subject*(session*trial*sample)

    elif setting.experiment_mode == "cross_session":
        m_eeg_data = [[[] for _ in range(len(sessions))]]
        ...
        for i in sessions:
            for idx1, (sub_eeg,sub_bio,sub_label) in enumerate(zip(eeg_data[i],bio_data[i], label[i])):
                for idx2, (trail_eeg, trial_bio, trial_label) in enumerate(zip(sub_eeg,sub_bio,sub_label)):
                    ...
                    m_label[0][i].extend(trial_label)#1*session*(subject*trial*sample)      
    assert setting.pr is None or (max(setting.pr)<=len(m_label) and min(setting.pr) > 0), \
        "primary rounds out of limit or primary rounds set less than 0"
    if setting.pr is not None:
        m_eeg_data = [m_eeg_data[i-1] for i in setting.pr]
        m_bio_data = [m_bio_data[i-1] for i in setting.pr]
        m_label = [m_label[i-1] for i in setting.pr]
    return m_eeg_data,m_bio_data, m_label